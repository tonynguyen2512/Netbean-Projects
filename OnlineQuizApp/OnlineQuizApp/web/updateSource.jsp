<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@page import="java.util.List"%>
<%@page import="org.onlinequizapp.daos.SourceDAO"%>
<%@page import="org.onlinequizapp.dtos.SourceDTO"%>
<%@page import="org.onlinequizapp.daos.LectureDAO"%>
<%@page import="org.onlinequizapp.dtos.LectureDTO"%>
<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
        <script type="application/x-javascript"> addEventListener("load", function() { setTimeout(hideURLbar, 0); }, false); function hideURLbar(){ window.scrollTo(0,1); } </script>
        <link href="CSS/style.css" rel="stylesheet" type="text/css" media="all" />
        <link href="//fonts.googleapis.com/css?family=Roboto:300,300i,400,400i,700,700i" rel="stylesheet">
        <!--<link rel='stylesheet' href='https://fonts.googleapis.com/css?family=Open+Sans'><link rel="stylesheet" href="./login.css">
        --><link href="CSS/nicepage.css" media="screen" rel="stylesheet" />
        <link href="CSS/Home.css" media="screen" rel="stylesheet" /><script class="u-script" type="text/javascript" src="JS/jquery-1.9.1.min.js" defer=""></script><script class="u-script" type="text/javascript" src="JS/nicepage.js" defer=""></script><meta name="generator" content="Nicepage 3.3.5, nicepage.com">
        <title>Update Question Information</title>
    </head>
    <body>
        <form action="SourceUpdateController" name="check" value="1" method="POST">
            <h1 align="center">UPDATE SOURCE ${sessionScope.LIST_SOURCE.sourceID}</h1>
            <div class="main-agileinfo">
                <div class="agileits-top">
                    <h2 style="text-align: center">Source Edit page</h2>
                    Source ID <input type="text" name="sourceID" value="${param.sourceID}" id="categoryID" readonly=""/></br>
                    <div>Lecture ID
                        <select name="lectureID" class="form-control" aria-label="Default select example">
                            <option selected>${param.lectureID}</option>
                            <c:if test="${requestScope.LIST_LECTURE != null && not empty requestScope.LIST_LECTURE}">
                                <c:forEach var="category" varStatus="counter" items="${requestScope.LIST_LECTURE}">
                                    <c:if test="${category.status==1}">
                                        <option value="${category.lectureID}">${counter.count} - ${category.lectureName}</option>
                                    </c:if>
                                </c:forEach>
                            </c:if>
                        </select>
                    </div><br>
                    <label>
                        <span>File Doc</span></br>
                        <input type="text" name="fileDoc" value="${param.fileDoc}" id="name" required=""/></br>
                    </label>
                    <label>
                        <span>File Pic</span></br>
                        <input type="text" name="filePic" value="${param.filePic}" id="name" required=""/></br>
                    </label>
                    <label>
                        <span>File Vid</span></br>
                        <input type="text" name="fileVid" value="${param.fileVid}" id="name" required=""/></br>
                    </label>
                    <label>
                        <span>Reference</span></br>
                        <input type="text" name="reference" value="${param.reference}" id="answer" required=""/></br>
                    </label>
                    <label>
                        <span>Status (1,0)</span></br>
                        <input type="text" name="status" value="${param.status}" id="status" required=""/></br>    
                        <input type="hidden" name="search" value="${param.search}" required=""/></br>
                        <input type="hidden" name="cate" value="${param.cate}"/>
                        <input type="hidden" name="check" value="${param.check}"/>
                        <input type="submit" name="action" value="Confirm Update Source"/>
                    </label>
                </div>
            </div>
        </form>
        <ul class="colorlib-bubbles">
            <li></li>
            <li></li>
            <li></li>
            <li></li>
            <li></li>
            <li></li>
            <li></li>
            <li></li>
            <li></li>
            <li></li>
        </ul>
    </body>
</html>